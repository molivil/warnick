...
# - tee, cut, grep, cat, date, wget, curl
#
# OTHER PREREQUISITES:
...
if [[ -z "$1" ]]; then
  ...
  echo "  feature is reserved for use in a multi-user environment, such as on the"
  echo "  Protoweb development server. If you are unsure, do not use it."
  echo
  exit 0
fi
...
if [[ "$owner" != "$defaultowner" ]]; then
  ...
  if [[ ! -z "$host" ]]; then rm -r ./sites/$host; fi
fi
