...
#    printf "`date +"%d.%m.%Y %T"` GET($pl): http://$host/$path\n" 2>&1 |tee -a $logfile
    ...
    if [ -f "./sites/$host/$path" ] || [ -f "./sites/$host/$path/index.html" ] || [ -f "./sites/$host/$path/index.htm" ] || [ -f "./sites/$host/$path/index.shtml" ] || [ -f "./sites/$host/$path/index.asp" ]; then
      ...
      if [[ $archstatus == "302" ]]; then
        ...
        log 3 "http://$host/$path 302 - Resource found on a different date ($archfounddate)"
        # Get location field
        if [[ "${archfounddate:0:8}" == "${d:0:8}" ]]; then
          log 2 "http://$host/$path 302 - Alt found from same day (${archfounddate:0:8}) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:6}" == "${d:0:6}" ]]; then
          log 2 "http://$host/$path 302 - Alt found within same month (${archfounddate:0:6}) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "${d:0:4}" ]]; then
          log 2 "http://$host/$path 302 - Alt found within same year (${archfounddate:0:4}) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "$(expr ${d:0:4} - 1)" ]]; then
          log 2 "http://$host/$path 302 - Alt found for previous year ($(expr ${d:0:4} - 1)) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "$(expr ${d:0:4} - 2)" ]]; then
          log 2 "http://$host/$path 302 - Alt found for target year - 2 ($(expr ${d:0:4} - 2)) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "$(expr ${d:0:4} - 3)" ]]; then
          log 2 "http://$host/$path 302 - Alt found for target year - 3 ($(expr ${d:0:4} - 3)) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "$(expr ${d:0:4} + 1)" ]]; then
          log 2 "http://$host/$path 302 - Alt found for target year + 1 ($(expr ${d:0:4} + 1)) (exact: $archfounddate)"
          altfound=1
        elif [[ "${archfounddate:0:4}" == "$(expr ${d:0:4} + 2)" ]]; then
          log 2 "http://$host/$path 302 - Alt found for target year + 2 ($(expr ${d:0:4} + 2)) (exact: $archfounddate)"
          altfound=1
        ...
        if [[ "$altfound" == "1" ]]; then
          ...
          log 1 "http://$host/$path 302 - No resource found close to target date. ($archfounddate)"
        fi
      fi
    fi
...
#  logfile="$tempdir/recovery.log"
  ...
  # Only move to complete directory if the process wasn't canceled
  if [ ! -f "$tempdir/cancel-job" ]; then
    mkdir -p "./temp/complete/warnick-$wid"
    mv $tempdir ./temp/complete/
  else
    touch $tempdir/cancel-job-success
    chown :www-data $tempdir/cancel-job-success 2>&1
    chmod 770 $tempdir/cancel-job-success 2>&1
  fi
fi