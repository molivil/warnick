...
function geturl {
  ...
  if [[ $archstatus == "403" ]] || [[ $archstatus == "502" ]]; then
    log 1 "Warning: Encountered a status code indicating possible throttling. Cooling down for 60 seconds..."
    sleep 60
  fi
...
if [[ -z "$startpath" ]]; then
  # 2.1.2 Path is empty, so this is likely the domain's homepage. Add it to the list.
  echo "$depth,$host/index.html" >> $linkfile
fi
