...
echo
echo "Developed by Oliver Molini for ProtoWeb.org"
    ...
    if [ -f "./sites/$host/$path" ] || [ -f "./sites/$host/$path/index.html" ] || [ -f "./sites/$host/$path/index.htm" ] || [ -f "./sites/$host/$path/index.shtml" ] || [ -f "./sites/$host/$path/index.asp" ]; then
      ...
      if [[ $archstatus == "302" ]]; then
        ...
        if [[ "$path$ext" == *.html || "$path$ext" == *.htm || "$path$ext" == *.asp || "$path$ext" == *.shtml ]]; then
          # File is a HTML document. Only download if it is near target date.
          # Get location field
          ...
          # File does not end with .html, we can attempt to download the alternative.
          altfound=1
        fi
        if [[ "$altfound" == "1" ]]; then
          #echo "wget --quiet --max-redirect=1 -e robots=off -nH -nc -P $tempdir/web $archurl 2>&1 |tee -a $logfile"
